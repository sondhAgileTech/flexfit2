...
class ContractWarranty2Controller extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->disableExport();
        $grid->model()->where(DB::raw('DATE_ADD(finish_date, INTERVAL 6 MONTH)'), '>', Carbon::now())->orderBy('finish_date', 'asc');
        ...
        $grid->finish_date('Trạng thái bảo hành')->display(function ($date) {
            ...
            $time_maintain_1 = date('d/m/Y', strtotime($date.' + 6 months'));
        ...
        $grid->column('Xem chi tiết')->display(function () {
            return '<a href="/admin/contracts_warranty_2/'.$this->id.'">Xem Các sản phẩm</a>';
        });
        ...
        $show->products('Sản Phẩm')->as(function ($products) {
            if(is_array($products))
            {
                ...
                foreach ($products as $item) {
                    ...
                    $date2 = $countdown2 >0 ? '<span style="color:red;">'.date('d/m/Y', strtotime($this->finish_date.' + 6 months')).'</span>' : '<span style="text-decoration: line-through;">'.date('d/m/Y', strtotime($this->finish_date.' + 6 months')).'</span>';
                    if($data) {
                        $html .= '<tr>
                          <th scope="row">'.$i.'</th>
                          <td>'.$data->name.'</td>
